export const SITE = {
  website: "https://monopolies.net/", // replace this with your deployed domain
  author: "Data Rights",
  profile: "https://monopolies.net/",
  repository: "https://github.com/monopolies/",
  description: "Know which brands belong to big companies, and who owns your digital life.",
  title: "Digital Monopolies",
  //ogImage: "favicon.svg",
  lightAndDarkMode: false,
  lang: "en", // html lang code. Set this empty and default will be "en"
  timezone: "Europe/Berlin", // Default global timezone (IANA format) https://en.wikipedia.org/wiki/List_of_tz_database_time_zones
  orbit1RevolveTime: 12, // in seconds, only used in view, not in CSS
  orbit2RevolveTime: 16, // in seconds, only used in view, not in CSS
  orbit3RevolveTime: 24, // in seconds, only used in view, not in CSS
} as const;
